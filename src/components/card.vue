<template>
  <span>
    <v-toolbar>
      ...
      <v-toolbar-title>Информация о пропуске <span v-if="!items.loading">№ {{ (parseInt(items.DATA_CARD.ZPROPUSK)).toString().substr(3) || parseInt(items.DATA_CARD.DOKNR) }}</span></v-toolbar-title>
      <v-spacer></v-spacer>
